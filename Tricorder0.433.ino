/*
// -- Tricorder v0.43.3 - By Tony Dillberg
...
// -- franchise.
// --
// -- Added GPS Time and Date on Main Screen
// -- Minor Code Clean up
// -- Changed a few colors and added more color definitions
// -- 
// -- Main Menu a bit laggy now
*/
...
// --- Menu options ---
const char* menuOptions[8] = {"1.Atmo", "2.Nav", "3.dB", "4.MDet", "5.EMF", "6.irRec", "7.irSnd", "8.Batt"};
int menuCursor = 0; // 0 to 7
...
TimeChangeRule usCDT = {"CDT", Second, Sun, Mar, 2, -300}; // DST starts 2nd Sun March at 02:00
TimeChangeRule usCST = {"CST", First, Sun, Nov, 2, -360}; // Std starts 1st Sun Nov at 02:00
...
//#define IR_SEND_PIN 3
//#define IR_USE_SAMD_TIMER
int irSendCursor = 0; // 0=PWR, 1=VolUP, 2=VolDN
...
void setup() {
  ...
  //pinMode(IR_SEND_PIN, OUTPUT); // output pin for IR Send
  IrSender.begin(IR_SEND_PIN); // output pin for IR Send
  //IrSender.enableIROut(38); // Set frequency 38kHz for IR Send
...
void handleMenu(int pressType) {
  ...
  if (pressType == SHORT_PRESS) {
    if (lastButton == 2) { // Up
      menuCursor = (menuCursor - 1 + 8) % 8;  
    } else if (lastButton == 4) { // Down
      menuCursor = (menuCursor + 1) % 8;     
    } else if (lastButton == 5) { // Left
      if (menuCursor >= 4) menuCursor -= 4;   
